...
// ── Shared layout ──────────────────────────────────────────────────────────────
function Layout({ bubble, mascotSize = 160, children, button }) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, width: '100%' }}>
        ...
        <div style={{
          ...
          <p style={{
            ...
            color: '#1a1a1a', margin: 0, lineHeight: 1.4, textAlign: 'center',
          }}>{bubble}</p>
          {/* Tail pointing down */}
          <div style={{
          ...
          <div style={{
            ...
            borderTop: '14px solid #e5e7eb', zIndex: -1,
          }} />
        </div>
        ...
        <div style={{ animation: 'mascot-float 2.2s ease-in-out infinite' }}>
          <img src="/onboarding-mascot.png" alt="Numio"
            style={{ width: mascotSize, height: 'auto', display: 'block' }} />
        </div>

        {/* Extra content below mascot */}
        {children && (
  ...
  return (
    <input
      ...
      style={{
        ...
        border: `2.5px solid ${focused ? '#58cc02' : '#e5e7eb'}`,
        outline: 'none',
...
export default function ChildOnboarding({ kidId, parentId, onDone, startStep = 0 }) {
  ...
  const [currentOpIdx, setCurrentOpIdx] = useState(0)
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          <StyledInput
            ...
            placeholder="Coin price (150–250)..."
          />
  ...
  // SCREEN 4: How it works (animated)
  if (step === 4) return (
    <Layout
      bubble="Here's how Numio works! 😄"
      button={
  ...
  if (step === 5) return (
    <Layout
      bubble={`Now let's see what you already know, ${name}! 👀`}
  ...
  if (step === 6) return (
    <Layout
      ...
      button={
        <GreenButton
          onClick={() => {
            if (justStarting) saveAndFinish(false)
            else if (knownOps.length > 0) { setCurrentOpIdx(0); setStep(7) }
          }}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
        ...
        <div style={{
          ...
          <p style={{ fontFamily: "'Baloo 2', sans-serif", fontWeight: 700, fontSize: 20, color: '#1a1a1a', margin: 0, lineHeight: 1.4, textAlign: 'center' }}>
            Ooh nice, {name}! 🔥 Let's do a quick test to confirm your level!
          </p>
          <div style={{ position: 'absolute', bottom: -13, left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '11px solid transparent', borderRight: '11px solid transparent', borderTop: '13px solid white' }} />
          <div style={{ position: 'absolute', bottom: -16, left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '12px solid transparent', borderRight: '12px solid transparent', borderTop: '14px solid #e5e7eb', zIndex: -1 }} />
        </div>
        ...
        <img
          ...
          alt="Numio ready"
          style={{ width: '100%', maxWidth: 300, objectFit: 'contain', animation: 'pop 0.4s ease both' }}
        />
