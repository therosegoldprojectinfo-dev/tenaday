...
11. **`security_patch_v1.sql`** — Parent PIN column security
...
12. **`fix_quiz_results_grant.sql`** — quiz_results SELECT grant
    Grants SELECT on quiz_results to authenticated.
    Run AFTER get_quiz_results_rpc.sql.

13. **`security_patch_v2.sql`** — Correct parent_pin SELECT revoke
...
14. **`create_kid_rpc.sql`** — Secure kid creation RPC
