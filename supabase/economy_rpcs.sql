...
-- Prevents coin farming via exam replay (Revision screen)
-- Each exam can only award coins once per kid
...
-- Update complete_quiz_and_award_coins with idempotency guard
CREATE OR REPLACE FUNCTION complete_quiz_and_award_coins(
  p_exam_id uuid,
  p_kid_id  uuid
)
